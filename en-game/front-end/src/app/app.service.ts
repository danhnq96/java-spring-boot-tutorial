...
import { ACCESS_TOKEN, CART, USER_INFO } from "./theme/constants";
...
export class AppService {
  ...
  public getCarts(): void {
    if (this.authService.user.getValue()) {
      this.cartService.getCarts(this.authService.user.getValue().id);
    }
  }
  ...
  public addToCart(product: Product) {
    ...
    if (this.Data.cartList.filter((item) => item.productId == product.id)[0]) {
      const item = this.Data.cartList.filter((item) => item.productId == product.id)[0];

      if (item.cartCount >= product.availibilityCount) {
        this.snackBar.open(`Stock having ${product.availibilityCount} items. Can not add more item.`, "×", {
          panelClass: ["error"],
    ...
    this.cartService.addCart(this.authService.user.getValue().id, cart);
  }

  public updateCartCount(product: Cart) {
    this.cartService.update(product);
  }

  public resetProductCartCount(product: Cart) {
    product.cartCount = 0;
    ...
    const cart = this.Data.cartList.filter((item) => item.id === product.id)[0];
    if (cart) {
  ...
  public clearCarts() {
    this.Data.compareList.forEach((item) => {
      item.cartCount = 0;
    });
    this.Data.wishList.forEach((item) => {
      item.cartCount = 0;
    });
    this.cartService.clearAllCarts(this.authService.user.getValue().id);
  }
